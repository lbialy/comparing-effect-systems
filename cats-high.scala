...
final class CatsScraperHighLevel(
  ...
  def start: IO[Unit] =
    for
      ...
      _ <- IO.println("CatsScraperHighLevel: Finished.")
    yield ()

  private def worker(queue: Queue[IO, Scrape | Done], visited: Ref[IO, Set[Uri]], inFlight: Ref[IO, Int]): IO[Unit] =
    queue.take.flatMap {
      case Done => IO.println("CatsScraperHighLevel: Received poison pill.") *> IO.unit
      case Scrape(uri, depth) =>
        given trace: Trace = Trace(uri)
        val handleUri =
          if depth >= maxDepth then IO.println(s"$trace: CatsScraperHighLevel: depth limit – skipping $uri")
          else
            visited.getAndUpdate(_ + uri).flatMap { visitedSet =>
              if visitedSet.contains(uri) then IO.println(s"$trace: CatsScraperHighLevel: already visited – skipping $uri")
  ...
  private def crawl(uri: Uri, depth: Int, queue: Queue[IO, Scrape | Done], inFlight: Ref[IO, Int])(using
    ...
    for
      _ <- IO.println(s"$trace: CatsScraperHighLevel: crawling $uri")
